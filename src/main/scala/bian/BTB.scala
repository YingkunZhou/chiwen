...
class BTB(implicit conf: CPUConfig) extends Module with BTBParams {
  val io = IO(new Bundle{
    ...
    val predict = Output(Vec(conf.nInst, Valid(new Predict(conf.xprlen))))
  ...
  io.split := io.predict(0).bits.redirect && !io.if_pc(OFF_LSB).toBool
  for (i <- 0 until conf.nInst) {
    ...
    io.predict(i).valid := predict.low_val(i).orR
    io.predict(i).bits.redirect := io.predict(i).valid &&
      (Mux1H(predict.low_val(i), btb.bj_type) =/= BTBType.branch.U ||
       Mux1H(predict.low_val(i), btb.h_count)(1).toBool)
    io.predict(i).bits.tgt := Mux(io.predict(i).bits.redirect, predict.jump_tgt(i), predict.cont_tgt(i))
  }
