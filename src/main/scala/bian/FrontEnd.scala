...
import chisel3._
import chisel3.util.{Cat, Fill, Valid}
...
class FrontEnd(implicit conf: CPUConfig) extends Module with BTBParams {
  ...
  when (io.back.forward(1)) {
    ...
    bj.jal  := Mux(dec_isbj(0), microDec(0).isjal, microDec(1).isjal)
  ...
  val bj_tgt = bj.pc + sext_imm
  val exe_mispredict = bj_tgt =/= bj.info.pred.tgt && bj.info.pred.redirect
  ...
  io.back.info.pred.tgt := Mux(bj.info.branch, bj_tgt, bj.info.pred.tgt)
  io.back.info.pred.redirect := bj.info.pred.redirect
