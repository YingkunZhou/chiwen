...
class AsyncScratchPadMemory(val num_core_ports: Int,val num_bytes: Int = (1 << 21))(implicit val conf: CPUConfig) extends Module
{
...
class SyncScratchPadMemory(val num_core_ports: Int, val num_bytes: Int = (1 << 21))(implicit val conf: CPUConfig) extends Module
{
   val io = IO(new Bundle
   {
      val core_ports = Vec(num_core_ports, Flipped(new MemPortIo(data_width = conf.xprlen)) )
